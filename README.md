...
仅在开发模式下访问 `http://localhost:5173/?stress=1`，会临时追加 14 条超长名称的站点和项目，用于检查大量条目、超长文字与列表收纳。普通宽度下主页每组最多展示 4 项；窄屏或两组均超量时每组最多展示 3 项，其余通过“查看全部”进入独立的可滚动列表。生产构建会强制关闭该模式：部署后的站点即使带上 `?stress=1` 也不会显示测试数据。
